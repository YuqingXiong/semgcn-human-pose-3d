    ...
    for subject in keypoints.keys():
        for action in keypoints[subject]:
            for cam_idx, kps in enumerate(keypoints[subject][action]):
                ...
                kps[..., :2] = normalize_screen_coordinates(kps[..., :2], w=cam['res_w'], h=cam['res_h'])   # 坐标处理！！！！！！！
                keypoints[subject][action][cam_idx] = kps   # keypoints是S**里面的action**的哪个相机方位索引 = 16个关节点的2维坐标=========
...
def fetch(subjects, dataset, keypoints, action_filter=None, stride=1, parse_3d_poses=True):
    ...
    for subject in subjects:
        for action in keypoints[subject].keys():
            ...
            poses_2d = keypoints[subject][action]   # poses_2d是所有相机方位cam_idx的16个关节点二维坐标========================
            for i in range(len(poses_2d)):  # Iterate across cameras
                out_poses_2d.append(poses_2d[i])    # out_poses_2d就是poses_2d============================================
