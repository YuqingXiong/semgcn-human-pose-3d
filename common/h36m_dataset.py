...
class Human36mDataset(MocapDataset):
    ...
    def define_actions(self, action=None):  # 动作的定义
        ...
        if action is None:  # action为空返回所有的动作名称
            return all_actions

        if action not in all_actions:   # 如果动作不在定义的动作里，提示错误信息
            raise (ValueError, "Undefined action: {}".format(action))   # 未定义某某动作

        return [action] # 返回这个动作例如["Walking"]
